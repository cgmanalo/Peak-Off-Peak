...
const byte interruptPin = 3; //changed from 2 to 3
...
void setup() {
    ...
    //attachInterrupt(digitalPinToInterrupt(2), checkButtonISR, RISING);
}
...
float checkBatteryVoltageLevel(){
  ...
  return (batLevel/1023)*20;
}
*/
//VOLTAGE READER VERSION
...
float checkBatteryCurrentLevel(){
  ...
  float batLevelSample;
  for (int i = 0; i < 20; i++){
    batLevelSample =analogRead(battCurrentLevel);
    batLevel = batLevel + batLevelSample/20;
    delay (10);
  }
  return ((8/(3.39-2.5562))*((batLevel*5/1023)-2.5562));
}
*/
...
void updateHost(){
  Serial.println("STATUS|" + (String)statusArray);
  Serial.println("BATLVL|" + (String)BatteryVoltageLevel);
  Serial.println("BATAMP|" + (String)BatteryCurrentLevel);
  Serial.println("WATTHR|" + (String)WattmeterLevel);
  Serial.print("CLOCK|");
  Serial.print(newNow.month(), DEC);
  Serial.print('-');
  Serial.print(newNow.day(), DEC);
  Serial.print('-');
  Serial.print(newNow.year(), DEC);
  Serial.print(' ');
  Serial.println(currentHour + ':' + currentMinute + ':' + currentSecond + ' ' + currentAMPM);
}
...
boolean resetWattHourIntegrator(){
  delay(1000);
  if (Serial1.available()){
    delay(10);
    //empty receive buffer 
  ...
  //send reset command once buffer is empty
  Serial1.write(0x02); // correct way to send [STX]
  Serial1.print("R"); // RESET WATT-HOUR INTEGRATOR
  Serial1.write(0x03); // [ETX] marker
  delay(1000);
  if (!Serial1.available()){
    return 1;
...
void serial1flush(){
  while(Serial1.available()){
    char flsh = Serial1.read();    
  }
}

void updateTotalWattHourMeter(){
  if (WattmeterLevel - InitWattmeterLevel > 0){
    totalRunningWattmeterReading = lastSavedWattmeterReading + WattmeterLevel - InitWattmeterLevel;
...
void updateTotalEnergyCost(float rate){
  if (WattmeterLevel - InitWattmeterLevel > 0){
    totalRunningEnergyCost = lastSavedEnergyCost + ((WattmeterLevel - InitWattmeterLevel)/1000)*rate;
